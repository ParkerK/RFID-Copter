...
% ***********************************************************
% throw noncritical error
% Used for: erroneous settings that would cause a function to fail (e.g. empty vectors)
%
% ATTENTION:
% Do not call err from a function that does not support version calls, because it tries to get the
...
% ***** Behavior *****
% err()
%    Like err('Unspecified error')
% err(message, a1, a2, ...)
%    Throws an error with identifier "SPR_sim:[throwing primary function]:noncritical" and additional text
%    "Noncritical error". Version information for all functions involved is added to stack.version (see
...
varargin{1} = sprintf('Noncritical error: %s', varargin{1});
...
if isempty(stack) % called from command line (unlikely, but still it shouldn't produce an error)
   ...
   message_struct = struct('message', sprintf(varargin{:}), ...
      'identifier', sprintf('SPR_sim:%s:noncritical', stack(1).file(1:end-2)), ...
      'stack', stack);
end
...
% % send an email
% send_email('Simulation terminated abnormally', error_msg);
