...
%
% ***** Behavior *****
% version = test_channel_large()
%    Just returns the version number (string).
% sumoferrors = test_channel_large(sumoferrors)
%    Tests the function channel_large, displays the results in the command window and returns the 
%    overall amount of errors found.
%
%
% ***** Interface definition *****
% function sumoferrors = test_channel_large(sumoferrors);
...
% output
disp('   = channel_large **');
disp('     checking gain and delay +/-1% for random setups');

% load settings and expected results
data = loadmat('results_channel_large.mat', '     ');
...
for i = 1 : length(results)
   ...
   % call channel_large
   [delay_s, gain] = channel_large(settings{i});
   ...
   %     check
   [errors, errortext] = check_result(i, delay_s, results{i}.delay_s, errors, errortext, 'delay_s', 'relerr', 0.01);
   [errors, errortext] = check_result(i, gain,    results{i}.gain,    errors, errortext, 'gain',    'relerr', 0.01);
