...
if strcmpi(settings.scenario, 'john');
   settings.suffix = 'john';
   % environment
   %     channel switches
   settings.channel_global.noise_on  = 0; % noise on ?
   settings.channel_global.surf_on   = 1; % reflective surfaces on ?
   ...
   % create two tags; position over time in two planes
   settings.tagpool = replicate(create_stdtag([0,0,1]), 2); % init at [0,0,1] (between all readers for initialization)
   npts = 20; % number of points in one dimension
   %    vertical surface
   [sy, sz] = meshgrid(linspace(-2,2.5,npts), linspace(0.5,2.5,npts));
   settings.loop.pos_t{1} = [zeros(numel(sy),1), sy(:), sz(:)];
   %    horizontal surface
   [sx, sy] = meshgrid(linspace(-2.5,2.5,npts), linspace(-2,2.5,npts));
   settings.loop.pos_t{2} = [sx(:), sy(:), 1*ones(numel(sy),1)];
   % num in front of ones is height
   % number here in front is the HEIGHT OFF THE FLOOR
   
   %     number of points
   settings.loop.n     = npts^2;
   settings.loop.n_sig = npts^2;
   ...
   settings.readerpool.pos      = {[0,-2.5,1]};
   % Position of reader - z coordinate is THE HEIGHT
